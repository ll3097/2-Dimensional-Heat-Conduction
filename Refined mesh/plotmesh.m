...
for i=1:nbe
    ...
    for i = 1:nel
        XX = [x(IEN(i,2)) x(IEN(i,3)) x(IEN(i,4)) x(IEN(i,2))];
        YY = [y(IEN(i,2)) y(IEN(i,3)) y(IEN(i,4)) y(IEN(i,2))];
        plot(XX,YY);hold on;

        if strcmpi(plot_nod,'yes')==1;   
            %text(XX(1),YY(1),sprintf('%0.5g',IEN(i,2)));
            %text(XX(2),YY(2),sprintf('%0.5g',IEN(i,3)));
            %text(XX(3),YY(3),sprintf('%0.5g',IEN(i,4)));
        end
    end
end
